...
class ShooterSubsystem(commands2.Subsystem):
    def __init__(self) -> None:
        ...
        self.left_motor.configure(configs.ShooterConfig.config, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters)
        self.right_motor.configure(configs.ShooterConfig.config, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters)
    ...
    def is_at_speed(self) -> bool:
        return math.isclose(self.target_shoot_speed + 100, self.left_motor.getEncoder().getVelocity(), rel_tol=constants.ShooterConstants.SHOOT_THRESHOLD)

    def get_shoot_power(self, distance: float) -> float:
        return constants.ShooterConstants.AUTO_SHOOT_SLOPE * distance + constants.ShooterConstants.AUTO_SHOOT_Y_INTERCEPT
