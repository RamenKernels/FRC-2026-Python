...
class VisionSubsystem(commands2.Subsystem):
    def __init__(self) -> None:
        ...
        self.photon_pose_estimator = PhotonPoseEstimator(constants.VisionConstants.APRIL_TAG_LAYOUT, constants.VisionConstants.ROBOT_TO_CAMERA_1)
    ...
    def get_estimated_global_pose(self) -> Optional[EstimatedRobotPose]:
        vision_estimate = self.photon_pose_estimator.estimateCoprocMultiTagPose(self.result)
        if vision_estimate is None:
            vision_estimate = self.photon_pose_estimator.estimateLowestAmbiguityPose(self.result)

        return vision_estimate

    def get_last_average_global_pose(self) -> Pose2d:
        return Pose2d(self.x_measurement_filter.lastValue(), self.y_measurement_filter.lastValue(), Rotation2d(self.rot_measurement_filter.lastValue()))
    ...
    def get_estimated_relative_pose(self) -> Pose2d | None:
        ...
        robot_pose: Pose3d = camera_pose.transformBy(constants.VisionConstants.ROBOT_TO_CAMERA_1.inverse())
    ...
    def periodic(self) -> None:
        ...
        if self.result.hasTargets():
            ...
            self.robot_to_target = constants.VisionConstants.ROBOT_TO_CAMERA_1 + best_target.getBestCameraToTarget()
            ...
            if (estimated_pose := self.get_estimated_global_pose()) is not None: 
                ...
                self.rot_measurement_filter.calculate(estimated_pose.estimatedPose.rotation().toRotation2d().degrees())
