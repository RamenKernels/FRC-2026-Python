...
class IntakeArmSubsystem(commands2.Subsystem):
    def __init__(self) -> None:
        ...
        self.arm_motor = SparkMax(constants.IntakeArmConstants.INTAKE_ARM_CAN_ID, SparkLowLevel.MotorType.kBrushless)
        self.arm_encoder = self.arm_motor.getEncoder()
        self.arm_pid = self.arm_motor.getClosedLoopController()

        self.arm_motor.configure(configs.IntakeArmConfig.config, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters)
    ...
    def manual_arm_command(self, power: Callable[[], float]) -> commands2.Command:
        return commands2.cmd.runEnd(lambda: self.setVolts(power() * constants.IntakeArmConstants.MANUAL_VOLTS), lambda: self.stopIntakeArm()) 
